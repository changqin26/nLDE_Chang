...
from querystructures import Query, GroupGraphPattern, TriplesBlock, TriplePattern, Argument

# Lexer

reserved = {'UNION': 'UNION', 'FILTER': 'FILTER', 'OPTIONAL': 'OPTIONAL', 'SELECT': 'SELECT', 'DISTINCT': 'DISTINCT',
            'WHERE': 'WHERE', 'PREFIX': 'PREFIX', 'LIMIT': 'LIMIT', 'OFFSET': 'OFFSET', 'ORDER': 'ORDER', 'BY': 'BY',
            'DESC': 'DESC', 'ASC': 'ASC', 'BOUND': 'BOUND', 'REGEX': 'REGEX', 'ISIRI': 'ISIRI', 'ISURI': 'ISURI',
            'ISBLANK': 'ISBLANK', 'ISLITERAL': 'ISLITERAL', 'LANG': 'LANG', 'DATATYPE': 'DATATYPE',
            'SAMETERM': 'SAMETERM', 'LANGMATCHES': 'LANGMATCHES', 'STR': 'STR', 'UCASE': 'UCASE', 'LCASE': 'LCASE',
            'CONTAINS': 'CONTAINS', 'UPPERCASE': 'UPPERCASE'}

tokens = [  # "RDFTYPE",
             "CONSTANT", "NUMBER", "VARIABLE", "LKEY", "RKEY", "COLON", "POINT", "COMA", "URI", "ALL", "LPAR", "RPAR",
             "EQUALS", "NEQUALS", "LESS", "LESSEQ", "GREATER", "GREATEREQ", "ID", "NEG", "AND", "PLUS", "MINUS",
             "TIMES", "DIV", "DOUBLE", "INTEGER", "DECIMAL", "FLOAT", "STRING", "BOOLEAN", "DATETIME", "NONPOSINT",
             "NEGATIVEINT", "LONG", "INT", "SHORT", "BYTE", "NONNEGINT", "UNSIGNEDLONG", "UNSIGNEDINT", "UNSIGNEDSHORT",
             "UNSIGNEDBYTE", "POSITIVEINT", "OR"] + list (reserved.values ())
...
def p_ggp_0(p):
    ...
    """
    if isinstance(p[2], TriplesBlock):
        p[0] = GroupGraphPattern(p[2])
    elif isinstance(p[2], GroupGraphPattern):
        p[0] = p[2]
    else:
        (l, r, join, union, optional) = p[2]
        p[0] = GroupGraphPattern(l, r, join, union, optional)
